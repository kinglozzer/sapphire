 ...
 * The `Requirements` class has been updated to make use of the `Injector` and `Config` APIs, the
   following methods/settings are marked as deprecated and will be removed in 4.0:
	* Requirements_Backend configuration:
		...
		- `Requirements::set_backend()` has been replaced with `Injector` configuration, see the [requirements reference documentation](/reference/requirements.md#extending--customising) for more information
	* Renamed methods:
